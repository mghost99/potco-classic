...
class LauncherBase(DirectObject):
    ...
    def __init__(self):
        ...
        if self.WIN32:
            self.VISTA = os.sys.getwindowsversion()[3] == 2 and os.sys.getwindowsversion()[0] == 6
        else:
            self.VISTA = 0
    ...
    def downloadMultifile(self, serverFilename, localFilename, mfname, callback, totalSize, currentSize,
                          callbackProgress):
    ...
    def getPercentPatchComplete(self, bytesWritten):
        return int(round(
            ((self.patchDownloadSoFar + bytesWritten) / float(self.totalPatchDownload)) * self.downloadPercentage))
    ...
    def downloadMultifileTask(self, task):
        ...
        if self.httpChannel.isValid() and self.httpChannel.isDownloadComplete():
            ...
            self.downloadMultifile(task.serverFilename, task.localFilename, task.mfname, task.callback, task.totalSize,
                                   bytesWritten, task.callbackProgress)
        elif (
                statusCode == 416 or statusCode == HTTPChannel.SCDownloadInvalidRange) and self.httpChannel.getFirstByteRequested() != 0:
        ...
        self.handleDownloadFatalError(task)
        self.downloadMultifile(task.serverFilename, task.localFilename, task.mfname, task.callback, task.totalSize, 0,
    ...
    def getMultifile(self, mfname):
        ...
        if not self.dldb.clientMultifileComplete(mfname) or not decompressedFilename.exists():
            ...
            if currentSize == 0:
                ...
                self.downloadMultifile(self.contentDir + mfname, localFilename, mfname, self.downloadMultifileDone,
                ...
                if clientHash.eq(serverHash):
                    self.notify.info('Multifile is not complete, finishing download for %s, size = %s / %s' % (
                    ...
                    self.curMultifileRetry += 1
                    self.notify.info(
                        'Multifile is not complete, and is out of date. ' + 'Restarting download with newest multifile')
                    self.dldb.setClientMultifileIncomplete(self.currentMfname)
                    self.dldb.setClientMultifileSize(self.currentMfname, 0)
    ...
    def decompressPatchDone(self):
        self.notify.info(
            'decompressPatchDone: Patching file: ' + self.currentPatchee + ' from ver: ' + repr(self.currentPatchVersion))
    ...
    def startReextractingFiles(self):
        self.notify.info('startReextractingFiles: Reextracting ' + repr(len(
            self.reextractList)) + ' files for multifile: ' + self.currentMfname)
    ...
    def patchMultifile(self):
        ...
        if clientVer == 1:
            ...
            self.notify.info(
                'patchMultifile: Old version for multifile: ' + self.currentMfname + ' Client ver: ' + repr(clientVer))
            self.maybeStartGame()
