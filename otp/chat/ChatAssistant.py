...
class ChatAssistant(DirectObject.DirectObject):
    ...
    def receiveGameMessage(self, message):
        ...
        if not isThought(message):
            self.historyOpen.insert(0, ChatMessage(self.chatTime(), GAMECHAT, message, None, None, None, 0, 0, 0))
    ...
    def receiveSystemMessage(self, message):
        error = None
        if not isThought(message):
            self.historyOpen.insert(0, ChatMessage(self.chatTime(), SYSTEMCHAT, message, None, None, None, 0, 0, 0))
    ...
    def receivePartyMessage(self, message):
        error = None
        if not isThought(message):
            self.historyOpen.insert(0, ChatMessage(self.chatTime(), PARTYCHAT, message, None, None, None, 0, 0, 0))
    ...
    def receiveFriendUpdate(self, friendId, friendName, isOnline):
        if isOnline:
            self.historyOpen.insert(0, ChatMessage(self.chatTime(), FRIEND_UPDATE, OTPLocalizer.FriendOnline % friendName, None, None, None, 0, 0, 0))
        else:
            self.historyOpen.insert(0, ChatMessage(self.chatTime(), FRIEND_UPDATE, OTPLocalizer.FriendOffline % friendName, None, None, None, 0, 0, 0))
        messenger.send('NewOpenMessage')
    ...
    def receiveGuildUpdate(self, memberId, memberName, isOnline):
        if base.cr.identifyFriend(memberId) is None:
            if isOnline:
                self.historyOpen.insert(0, ChatMessage(self.chatTime(), GUILD_UPDATE, OTPLocalizer.GuildMemberOnline % memberName, None, None, None, 0, 0, 0))
            else:
                self.historyOpen.insert(0, ChatMessage(self.chatTime(), GUILD_UPDATE, OTPLocalizer.GuildMemberOffline % memberName, None, None, None, 0, 0, 0))
            messenger.send('NewOpenMessage')

    def receiveAvatarOpenTypedChat(self, message, chatFlags, senderId, name = None):
        ...
        if not isThought(message):
            self.historyOpen.insert(0, ChatMessage(self.chatTime(), TYPEDCHAT, message, chatFlags, senderId, name, 0, 0, 0))
    ...
    def receiveAvatarOpenSpeedChat(self, type, messageIndex, senderId, name = None):
        error = None
        if not name and senderId:
            name = self.findName(senderId, 0)
        ...
        if senderId == base.localAvatar.doId:
            ...
            IAmSender = 0
        self.historyOpen.insert(0, ChatMessage(self.chatTime(), type, messageIndex, None, senderId, name, 0, 0, IAmSender))
        messenger.send('NewOpenMessage')
        return error
    ...
    def receiveAvatarWhisperTypedChat(self, message, senderId, name = None):
        ...
        self.historyReceivedWhisperAvatar[senderId].insert(0, ChatMessage(self.chatTime(), TYPEDCHAT, message, None, senderId, name, 0, 1, 0))
        self.historyOpen.insert(0, ChatMessage(self.chatTime(), TYPEDCHAT, message, None, senderId, name, 0, 1, 0))
        messenger.send('NewOpenMessage')
        return error
    ...
    def receiveAvatarWhisperSpeedChat(self, type, messageIndex, senderId, name = None):
        ...
        self.historyReceivedWhisperAvatar[senderId].insert(0, ChatMessage(self.chatTime(), type, messageIndex, None, senderId, name, 0, 1, 0))
        self.historyOpen.insert(0, ChatMessage(self.chatTime(), type, messageIndex, None, senderId, name, 0, 1, 0))
        messenger.send('NewOpenMessage')
        return error
    ...
    def receivePlayerOpenTypedChat(self, message, senderId, name = None):
        ...
        self.historyOpen.insert(0, ChatMessage(self.chatTime(), TYPEDCHAT, message, None, senderId, name, 1, 0, 0))
        messenger.send('NewOpenMessage')
        return error

    def receivePlayerOpenSpeedChat(self, type, messageIndex, senderId, name = None):
        error = None
        if not name and senderId:
            name = self.findName(senderId, 1)
        ...
        self.historyOpen.insert(0, ChatMessage(self.chatTime(), type, messageIndex, None, senderId, name, 1, 0, 0))
        messenger.send('NewOpenMessage')
        return error
    ...
    def receivePlayerWhisperTypedChat(self, message, senderId, name = None):
        ...
        self.historyReceivedWhisperPlayer[senderId].insert(0, ChatMessage(self.chatTime(), TYPEDCHAT, message, None, senderId, name, 1, 1, 0))
        self.historyOpen.insert(0, ChatMessage(self.chatTime(), TYPEDCHAT, message, None, senderId, name, 1, 1, 0))
        messenger.send('NewOpenMessage')
        return error
    ...
    def receivePlayerWhisperSpeedChat(self, type, messageIndex, senderId, name = None):
        ...
        self.historyReceivedWhisperPlayer[senderId].insert(0, ChatMessage(self.chatTime(), type, messageIndex, None, senderId, name, 1, 1, 0))
        self.historyOpen.insert(0, ChatMessage(self.chatTime(), type, messageIndex, None, senderId, name, 1, 1, 0))
        messenger.send('NewOpenMessage')
        return error
    ...
    def sendAvatarWhisperTypedChat(self, message, receiverId):
        ...
        if handle:
            ...
            self.historySentWhisperAvatar[receiverId].insert(0, ChatMessage(self.chatTime(), TYPEDCHAT, message, None, receiverId, self.findName(receiverId), 0, 1, 1))
            if self.logWhispers:
                self.historyOpen.insert(0, ChatMessage(self.chatTime(), TYPEDCHAT, message, None, receiverId, self.findName(receiverId), 0, 1, 1))
                messenger.send('NewOpenMessage')
            
        else:
            self.historySentWhisperAvatar[receiverId].insert(0, ChatMessage(self.chatTime(), AVATAR_UNAVAILABLE, OTPLocalizer.WhisperUnavailable, None, receiverId, '', 0, 1, 1))
            if self.logWhispers:
                self.historyOpen.insert(0, ChatMessage(self.chatTime(), AVATAR_UNAVAILABLE, OTPLocalizer.WhisperUnavailable, None, receiverId, '', 0, 1, 1))
                messenger.send('NewOpenMessage')
    ...
    def sendPlayerWhisperTypedChat(self, message, receiverId):
        ...
        self.historySentWhisperPlayer[receiverId].insert(0, ChatMessage(self.chatTime(), TYPEDCHAT, message, None, receiverId, self.findName(receiverId), 1, 1, 1))
        if self.logWhispers:
            self.historyOpen.insert(0, ChatMessage(self.chatTime(), TYPEDCHAT, message, None, receiverId, self.findName(receiverId, 1), 1, 1, 1))
            messenger.send('NewOpenMessage')
