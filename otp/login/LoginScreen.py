...
class LoginScreen(StateData.StateData, GuiScreen.GuiScreen):
    ...
    def load(self):
        ...
        self.failCreateAccountButton = DirectButton(parent = self.failDialog, relief = DGG.RAISED, borderWidth = (0.01,
          0.01), pos = (0, 0, linePos), scale = 0.9, text = OTPLocalizer.LoginScreenCreateAccount, text_scale = 0.06, text_pos = (0, -.02), command = self.__handleFailCreateAccount)
        linePos -= buttonLineHeight
    ...
    def handleLoginResponseMsg2(self, di):
        ...
        if di.getRemainingSize() > 0:
            self.cr.accountCreationDate = self.parseCreationDateStr(di.getString())
        else:
            self.cr.accountCreationDate = time.localtime()
        self.notify.info('Login response return code %s' % returnCode)
        if returnCode == 0:
            self.__handleLoginSuccess()
        elif returnCode == -13:
    ...
    def parseCreationDateStr(self, dateStr):
        if dateStr:
            result = time.strptime(dateStr, '%Y-%m-%d %H:%M:%S')
        else:
