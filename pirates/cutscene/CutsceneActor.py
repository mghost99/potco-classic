    ...
    def startCutscene(self, locators):
        ...
        if self.node() != NotImplemented:
            ...
            if not lodNode.isEmpty():
                lodNode.node().forceSwitch(characterDetailLevel)
    ...
    def sinkingVFX(self):
        ...
        if base.options.getSpecialEffectsSetting() >= base.options.SpecialEffectsMedium:
            ...
            if ripples:
                ...
                ripples.setPos(self.getPos(render))
        ...
        if base.options.getSpecialEffectsSetting() >= base.options.SpecialEffectsHigh:
            ...
            if splashes:
                ...
                splashes.setPos(self.getPos(render))
                splashes.setScale(scale / 60.0)
        ...
        if base.options.getSpecialEffectsSetting() >= base.options.SpecialEffectsHigh:
            ...
            if mist:
                ...
                mist.setPos(self.getPos(render))
                mist.setScale(scale / 50.0)
...
class CutJollyRoger(CutsceneActor, JollyRoger):
    ...
    def spawnSkeletons(self, cutscene):
        ...
        if spawnEffect:
            ...
            spawnEffect.setZ(spawnEffect.getZ(render) - 2.0)
        ...
        if spawnEffect2:
            ...
            spawnEffect2.setPos(jr2.find('**/dx_root').getPos(render))
            spawnEffect2.setZ(spawnEffect2.getZ(render) - 2.0)
...
class CutPirate(CutsceneActor, Pirate):
    ...
    def __init__(self, Uid, npcIndex, cutsceneDesc):
        ...
        if 'localAvatar' in __builtins__:
            self.generateHuman(dna.gender, base.cr.human)
        else:
...
class CutLocalPirate(CutsceneActor):
    ...
    def startCutscene(self, locators):
        ...
        self.localAvatar.disableMixing()
        if self.wantZombie:
    ...
    def finishCutscene(self):
        ...
        if hasattr(self.localAvatar, 'gameFSM'):
            ...
            if self.oldParams.gameState == 'EnterTunnel' or self.oldParams.gameState == 'DoorInteract':
