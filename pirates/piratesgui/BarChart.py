...
class BarChart(DirectFrame):
    ...
    def __init__(self, data, height, width, name = '', titleColor = (1.0, 1.0, 1.0, 1.0), maxValue = None):
        ...
        self.statBars = []
        self.title = DirectFrame(parent = self, relief = None, text = self.name, text_align = TextNode.ALeft, text_scale = PiratesGuiGlobals.TextScaleLarge, text_pos = (0.03, 0.01), text_fg = self.titleColor, text_shadow = PiratesGuiGlobals.TextShadow, textMayChange = 1, pos = (0, 0, self.height + 0.02))
    ...
    def addBar(self, data, repack = 1):
        meter = DirectWaitBar(parent = self, relief = DGG.RAISED, borderWidth = (0.004, 0.004), range = data[2], value = data[1], frameColor = (0, 0, 0, 0), barColor = (0.1, 0.7, 0.1, 1), frameSize = (0.17, self.width - 0.1, -(self.barHeight) * 0.25, self.barHeight * 0.75), text = str(data[1]), text_align = TextNode.ALeft, text_scale = 0.03, text_fg = (1, 1, 1, 1), text_shadow = (0, 0, 0, 1), text_pos = (0, 0), pos = (0, 0, 0))
        if self.maxValue:
