...
class FeedbackInput(GuiPanel.GuiPanel):
    ...
    def generateFeedbackPanel(self, commandToExe):
        self.feedbackInput = DirectEntry(parent = self, relief = DGG.GROOVE, scale = 0.051, pos = (0.007, 0, 0.42), borderWidth = PiratesGuiGlobals.BorderWidth, frameColor = (0, 0.0, 0.0, 0.5), text_align = TextNode.ALeft, width = 14, numLines = 9, focus = 1, cursorKeys = 1, text_fg = (1, 1, 1, 1), command = self.commandToExe, suppressKeys = 1, suppressMouse = 1, autoCapitalize = 0)
...
class FeedbackPanel(GuiPanel.GuiPanel):
    ...
    def __init__(self):
        ...
        self.message = DirectLabel(parent = self, relief = None, text = message, text_scale = PiratesGuiGlobals.TextScaleLarge, text_align = TextNode.ACenter, text_fg = PiratesGuiGlobals.TextFG1, text_shadow = PiratesGuiGlobals.TextShadow, text_wordwrap = 10, pos = (0.2225, 0, 0.93), textMayChange = 1)
    ...
    def __handleAccountWeb(self):
        base.popupBrowser(PLocalizer.URL_FeedbackFormManageAccountURL)
        self.feedbackObj.destroy()
        self.destroy()
