...
class MapPage(InventoryPage.InventoryPage):
    ...
    def createPortOfCall(self):
        ...
        self.portOfCallLabel = DirectLabel(parent = self, text = '', text_font = PiratesGlobals.getPirateOutlineFont(), text_scale = 0.045, text_fg = PiratesGuiGlobals.TextFG2, textMayChange = 1, pos = (0.55, 0, 0.08))
        self.portOfCallButton = GuiButton.GuiButton(parent = self, pos = (0.54, 0, 0.015), scale = 0.85, text = 'Return', text_scale = PiratesGuiGlobals.TextScaleLarge, text_pos = (0.033, -0.014), textMayChange = 1, image3_color = (0.8, 0.8, 0.8, 1), geom = teleportIcon, geom_pos = (-0.065, 0, 0), geom_scale = 0.2, command = self.handlePortOfCall)
    ...
    def createMapButtons(self, gui):
        z = 1.1
        self.worldButton = DirectButton(parent = self, relief = None, geom = (gui.find('**/topgui_icon_map_chart'), gui.find('**/topgui_icon_map_chart'), gui.find('**/topgui_icon_map_chart_over'), gui.find('**/topgui_icon_map_chart')), geom_scale = 0.5, pos = (0.34, 0, z), command = self.switchMode, extraArgs = [
            0])
        self.islandButton = DirectButton(parent = self, relief = None, geom = (gui.find('**/topgui_icon_map_island'), gui.find('**/topgui_icon_map_island'), gui.find('**/topgui_icon_map_island_over'), gui.find('**/topgui_icon_map_island')), geom_scale = 0.5, pos = (0.55, 0, z), command = self.switchMode, extraArgs = [
            1])
        self.townButton = DirectButton(parent = self, relief = None, geom = (gui.find('**/topgui_icon_map_town'), gui.find('**/topgui_icon_map_town'), gui.find('**/topgui_icon_map_town_over'), gui.find('**/topgui_icon_map_town')), geom_scale = 0.5, pos = (0.76, 0, z), command = self.switchMode, extraArgs = [
            2])
    ...
    def createWorldMap(self):
        ...
        if __dev__ and 0:
            ...
            self.mouseModeButton = DirectButton(parent = self, text = 'MouseMode', scale = 0.065, pos = (0.25, 0, 0.09), command = changeMouseMode)
            self.mouseModeLabel = DirectLabel(parent = self, scale = 0.075, pos = (0.5, 0, 0.087), text = repr(self.worldMap.mapBall.rMode), text_fg = (1, 1, 1, 1), textMayChange = 1)
    ...
    def createShardPanel(self):
        ...
        if __dev__ and 0:
            ...
            self.shardButton = DirectButton(parent = self, text = 'Show Shards', scale = 0.065, pos = (0.75, 0, 0.09), command = showShardList, textMayChange = 1)
