...
class PirateMemberButton(GuiButton.GuiButton):
    ...
    def __init__(self, owner, avId, playerId, mode, modeInfo = None):
        ...
        if self.mode == MODE_CREW_HUD:
            ...
            self.hpMeter = DirectWaitBar(parent = self, relief = DGG.RAISED, borderWidth = (0.004, 0.004), range = self.maxHp, value = self.hp, frameColor = (0.05, 0.05, 0.05, 1), barColor = (0.1, 0.7, 0.1, 1), pos = (0.15, 0, 0.012), frameSize = (0, 0.25, 0, 0.02), text = '%s/%s' % (self.hp, self.maxHp), text_align = TextNode.ALeft, text_scale = PiratesGuiGlobals.TextScaleTiny, text_fg = PiratesGuiGlobals.TextFG3, text_shadow = PiratesGuiGlobals.TextShadow, text_pos = (0.27, 0, 0.005), textMayChange = 1)
            ...
            self.statusLabel = DirectLabel(parent = self, relief = None, text = '', text_align = TextNode.ALeft, text_scale = PiratesGuiGlobals.TextScaleSmall, text_pos = (1.53, 0.02), text_fg = self.OnlineSubtextColor, text_shadow = PiratesGuiGlobals.TextShadow, textMayChange = 1, scale = 0.8)
        else:
            GuiButton.GuiButton.__init__(self, parent = self.owner.memberFrame.getCanvas(), text = '', text_scale = PiratesGuiGlobals.TextScaleMed, text_pos = (0.01, self.owner.memberOffset), text_align = TextNode.ALeft, text_fg = self.OfflineTextColor, text_shadow = PiratesGuiGlobals.TextShadow, textMayChange = 1, text_wordwrap = 40, image_scale = (self.owner.memberWidth, 1.0, self.owner.memberHeight * 2.62), image_pos = (0.21, 0.0, self.owner.memberHeight * 0.5), command = self.handlePress)
            self.hpMeter = DirectWaitBar(parent = self, relief = DGG.RAISED, borderWidth = (0.004, 0.004), range = self.maxHp, value = self.hp, frameColor = (0.05, 0.05, 0.05, 1), barColor = (0.1, 0.7, 0.1, 1), pos = (0.01, 0, 0.012), frameSize = (0, 0.25, 0, 0.02), text = '%s/%s' % (self.hp, self.maxHp), text_align = TextNode.ALeft, text_scale = PiratesGuiGlobals.TextScaleTiny, text_fg = PiratesGuiGlobals.TextFG3, text_shadow = PiratesGuiGlobals.TextShadow, text_pos = (0.256, 0, 0.005), textMayChange = 1)
            self.statusLabel = DirectLabel(parent = self, relief = None, text = '', text_align = TextNode.ALeft, text_scale = PiratesGuiGlobals.TextScaleSmall, text_pos = (0.03, 0.02), text_fg = self.OnlineSubtextColor, text_shadow = PiratesGuiGlobals.TextShadow, textMayChange = 1)
        self.initialiseoptions(PirateMemberButton)
        self.hpMeter.hide()
        self.statusLabel.hide()
        gui = loader.loadModel('models/gui/toplevel_gui')
        shipImage = (gui.find('**/topgui_icon_ship'), gui.find('**/topgui_icon_ship'), gui.find('**/topgui_icon_ship'))
        self.shipIcon = DirectButton(parent = self, relief = None, image = shipImage, image0_color = VBase4(1.0, 1.0, 1.0, 1.0), image1_color = VBase4(0.4, 0.4, 0.4, 1.0), image2_color = VBase4(0.0, 0.9, 0.0, 1.0), image_scale = 0.1, pos = (0.35, 0, 0.035), command = self.handleShipPress)
    ...
    def __cmp__(self, other):
        ...
        if self.mode == MODE_FRIEND_PLAYER and other.mode == MODE_FRIEND_AVATAR:
            return -1
        elif self.mode == MODE_FRIEND_AVATAR and other.mode == MODE_FRIEND_PLAYER:
            return 1
        else:
    ...
    def update(self, modeInfo = None):
        ...
        if self.mode == MODE_CREW_HUD:
            self.configure(state = state, text = text, text_fg = text_fg, image_color = buttonColor, text_pos = (0.15, 0.04, 3.45))
        elif self.mode == MODE_CREW_HUD_SEA:
            self.configure(state = state, text = text, text_fg = text_fg, image_color = buttonColor, text_pos = (0.15, 0.04, 1.5), text_scale = 0.0375)
        else:
    ...
    def handlePress(self):
        if self.mode == MODE_FRIEND_PLAYER:
            messenger.send(PiratesGlobals.PlayerDetailsEvent, [
                self.playerId])
        elif self.mode in (MODE_FRIEND_AVATAR, MODE_GUILD, MODE_CREW, MODE_CREW_HUD, MODE_CREW_HUD_SEA):
