...
class LookoutRequestLVL1(InventoryPage):
    ...
    def addShipOptions(self, options, callback):
        ...
        def receiveInventory(inventory):
            ...
            for currShipId in shipIds:
                ...
                shipConfig = ShipGlobals.getShipConfig(shipView.shipClass)
                newMaxCrew = shipConfig['setMaxCrew'][0]
    ...
    def determineLvl2ItemList(self, category, callback = None, gameType = None):
        ...
        if gameType != None:
            ...
            GameTypeGlobals.getGameStyles(category, None, gotStylz)
    ...
    def createRequestIface(self):
        if self.UI_VERSION == 0:
            ...
            (self.backButton, self.backButtonText) = self.createButtonAndText(imageInfo = {
                ...
                'buttonScale': 0.3,
                'clickCommand': self.backClick}, textInfo = PLocalizer.LookoutBack)
        self.activityListItems = ListFrame(0.8, None, 'blah', self, frameColor = (0, 0, 0, 0))
        ...
        self.activityList = DirectScrolledFrame(parent = self, frameSize = size, relief = DGG.GROOVE, state = DGG.NORMAL, frameColor = (0, 0, 0, 0), borderWidth = PiratesGuiGlobals.BorderWidth, canvasSize = (0, 0.7, 0, self.activityListItems['frameSize'][3]), verticalScroll_image = charGui_slider, verticalScroll_frameSize = (0, PiratesGuiGlobals.ScrollbarSize, 0, self.height * 0.8), verticalScroll_thumb_image = charGui_slider_thumb, sortOrder = 5, pos = pos)
        if self.UI_VERSION == 0:
    ...
    def updateTitle(self, mode = PiratesGuiGlobals.REQUEST_CAT_MODE):
        ...
        self.titleText = DirectLabel(parent = self, relief = None, text = titleText, text_align = TextNode.ALeft, text_scale = 0.09, text_fg = PiratesGuiGlobals.TextFG1, text_shadow = PiratesGuiGlobals.TextShadow, textMayChange = 1, pos = (0.36, 0, 1.17))
        toplevelUI = loader.loadModel(self.TOPLEVEL_GUI_FILE)
        if self.titleImage:
            self.titleImage.removeNode()
        
        self.titleImage = OnscreenImage(image = toplevelUI.find('**/' + iconName), pos = (0.25, 0, 1.2), scale = (0.6, 0.6, 0.6), parent = self)
        if self.descText:
            self.descText.removeNode()
        
        self.descText = DirectLabel(parent = self, relief = None, text = titleDesc, text_align = TextNode.ALeft, text_scale = 0.045, text_fg = PiratesGuiGlobals.TextFG1, text_shadow = PiratesGuiGlobals.TextShadow, text_wordwrap = 15, textMayChange = 1, pos = (0.235, 0, 1.06))
    
    def createListFrame(self, list, lookoutUI = None):
        self.activityListBorderFrame = BorderFrame(parent = list, pos = (0.4, 0, 0.265), scale = (0.8, 1, 0.6))
    ...
    def createFoundIface(self):
        ...
        self.foundType = DirectLabel(parent = self, relief = None, text = PLocalizer.LookoutFoundStatusType, text_align = TextNode.ACenter, text_scale = 0.05, text_fg = PiratesGuiGlobals.TextFG1, text_shadow = PiratesGuiGlobals.TextShadow, textMayChange = 1, pos = (0.52, 0, 0.82))
        self.foundChance = DirectLabel(parent = self, relief = None, text = PLocalizer.LookoutFoundStatusChance, text_align = TextNode.ACenter, text_scale = 0.045, text_fg = PiratesGuiGlobals.TextFG1, text_shadow = PiratesGuiGlobals.TextShadow, textMayChange = 1, pos = (0.52, 0, 0.4))
        lookoutUI = loader.loadModel(self.LOOKOUT_GUI_FILE)
        ...
        self.TimerDisplay = DirectLabel(parent = self, relief = None, text = PLocalizer.LookoutTimer % self.joinTimeout, text_align = TextNode.ACenter, text_scale = 0.055, text_fg = PiratesGuiGlobals.TextFG1, text_shadow = PiratesGuiGlobals.TextShadow, textMayChange = 1, pos = (0.57, 0, 0.4))
        self.TimerDisplay.hide()
