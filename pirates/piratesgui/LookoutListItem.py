...
class LookoutListItem(ButtonListItem.ButtonListItem):
    ...
    def _createIface(self):
        ...
        yOff = 0.065
        if self.imageRef:
            self.icon = OnscreenImage(image = self.imageRef, pos = (0.1, 0, 0.082), scale = (0.6, 0.6, 0.6), parent = self)
            xOff = 0.2
            wordwrap = 15
    ...
    def createListFrame(self):
        self.clearListFrame()
        self.activityListBorderFrame = BorderFrame(parent = self, pos = (0.375, 0, 0.082), scale = (0.75, 1, 0.165), borderScale = 0.2)
