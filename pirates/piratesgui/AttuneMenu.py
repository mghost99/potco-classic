...
class AvatarInfoButton(GuiButton):
    ...
    def __init__(self, owner, avId):
        ...
        self.hp = 0
        self.maxHp = 0
        GuiButton.__init__(self, text = '', text_scale = PiratesGuiGlobals.TextScaleSmall, text_pos = (0.025, 0.085), text_align = TextNode.ALeft, text_fg = self.OnlineTextColor, text_shadow = PiratesGuiGlobals.TextShadow, textMayChange = 1, text_wordwrap = 14, relief = None, borderWidth = PiratesGuiGlobals.BorderWidthSmall, frameColor = (0.45, 0.45, 0.35, 1.0), image_scale = (0.45, 1, 0.2), image_pos = (0.215, 0.0, 0.08), command = self.select)
        self.initialiseoptions(AvatarInfoButton)
        self.hpMeter = DirectWaitBar(parent = self, relief = DGG.RAISED, borderWidth = (0.004, 0.004), range = 50, value = 20, frameColor = (0.05, 0.35, 0.05, 1), barColor = (0.1, 0.7, 0.1, 1), pos = (0.015, 0, 0.06), frameSize = (0, 0.25, 0, 0.018), text = '%s/%s' % (self.hp, self.maxHp), text_align = TextNode.ALeft, text_scale = PiratesGuiGlobals.TextScaleMicro, text_fg = PiratesGuiGlobals.TextFG2, text_shadow = PiratesGuiGlobals.TextShadow, text_pos = (0.256, 0, 0.005), textMayChange = 1)
...
class AttuneMenu(DirectFrame):
    ...
    def __init__(self, parent = base.a2dRightCenter, command = None, draggable = 0, **kw):
        ...
        wordwrap = 13
        self.titleLabel = DirectLabel(parent = self, relief = None, pos = (0.05, 0, self.HEIGHT - PiratesGuiGlobals.TextScaleMed * 2.5), text = PLocalizer.UnattuneGui, text_align = TextNode.ALeft, text_scale = titleFont, text_pos = (0.015, 0.05), text_fg = textColor, text_shadow = textShadow, text_font = PiratesGlobals.getPirateOutlineFont(), textMayChange = 1, text_wordwrap = wordwrap, sortOrder = 21)
        self.buttons = {}
        self.updateButton(0)
        self.hide()
