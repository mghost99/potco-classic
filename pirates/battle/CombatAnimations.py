...
class CombatAnimations:
    ...
    def getThrust(self, av, skillId, ammoSkillId, charge, target):
        ...
        ival = Parallel(Sequence(Func(self.lockInput, av),
                                 Func(av.currentWeapon.setTrailLength, 0.5),
                                 Func(av.currentWeapon.beginAttack, av),
                                 Func(av.currentWeapon.playSkillSfx, skillId, av), av.actorInterval('cutlass_combo', playRate = 1.0, startFrame = 151, endFrame = 164, blendInT = 0.5, blendOutT = 0),
                                 Func(av.currentWeapon.showSpinBlur), av.actorInterval('cutlass_combo', playRate = 1.0, startFrame = 165, endFrame = 170, blendInT = 0, blendOutT = 0),
                                 Func(startVFX), av.actorInterval('cutlass_combo', playRate = 1.0, startFrame = 171, endFrame = 175, blendInT = 0, blendOutT = 0),
                                 Func(av.currentWeapon.hideSpinBlur), av.actorInterval('cutlass_combo', playRate = 1.0, startFrame = 176, endFrame = 210, blendInT = 0, blendOutT = 0.5),
                                 Func(av.currentWeapon.endAttack, av)), Sequence(Wait(1.3), Func(self.unlockInput, av)))
        return ival
    ...
    def getSweep(self, av, skillId, ammoSkillId, charge, target):
        ...
        ival = Sequence(Func(self.lockInput, av),
                        Func(av.motionFSM.off),
                        Func(av.currentWeapon.hideMouse, av),
                        Func(av.currentWeapon.setTrailLength, 0.7),
                        Func(av.currentWeapon.beginAttack, av),
                        Func(av.currentWeapon.playSkillSfx, skillId, av), av.actorInterval('cutlass_sweep', playRate = 1.0, startFrame = 1, endFrame = 10, blendOutT = 0),
                        Func(startVFX), av.actorInterval('cutlass_sweep', playRate = 1.0, startFrame = 11, endFrame = 15, blendInT = 0, blendOutT = 0),
                        Func(startVFX2), av.actorInterval('cutlass_sweep', playRate = 1.0, startFrame = 16, endFrame = 35, blendInT = 0),
                        Func(av.currentWeapon.endAttack, av),
                        Func(av.motionFSM.on),
                        Func(self.unlockInput, av))
        return ival
    ...
    def getDaggerThrowDirtInterval(self, av, skillId, ammoSkillId, charge, target):
        ...
        ival = Sequence(Func(self.lockInput, av),
                        Func(av.motionFSM.off),
                        Func(av.currentWeapon.hideMouse, av),
                        Func(base.disableMouse),
                        Func(av.currentWeapon.endAttack, av),
                        Func(av.currentWeapon.setTrailLength, 0.25),
                        Func(av.currentWeapon.hideWeapon),
                        Func(av.currentWeapon.beginAttack, av), av.actorInterval('dagger_throw_sand', playRate = 1.0, startFrame = 1, endFrame = 10, blendInT = 0.2, blendOutT = 0),
                        Func(av.currentWeapon.playSkillSfx, skillId, av),
                        Func(startVFX), av.actorInterval('dagger_throw_sand', playRate = 1.0, startFrame = 11, endFrame = 38, blendInT = 0, blendOutT = 0.3),
                        Func(av.currentWeapon.showWeapon),
                        Func(av.motionFSM.on),
                        Func(self.unlockInput, av))
        return ival
    ...
    def getChargeBanishAnim(self, av, skillId, ammoSkillId, charge, target):
        ...
        def startVFX():
            ...
            if av.currentWeapon.effect and not av.currentWeapon.isEmpty():
                av.currentWeapon.effect.reparentTo(av.currentWeapon)
                av.currentWeapon.effect.setPos(av.currentWeapon, offset + Vec3(0, 1.45, -.1))
                av.currentWeapon.effect.setEffectColor(av.currentWeapon.getEffectColor(av.currentWeapon.itemId))
                av.currentWeapon.effect.startLoop()
            ...
            if base.options.getSpecialEffectsSetting() >= base.options.SpecialEffectsMedium:
                ...
                if av.currentWeapon.effect2 and not av.currentWeapon.isEmpty():
                    av.currentWeapon.effect2.reparentTo(av.currentWeapon)
                    av.currentWeapon.effect2.setPos(av.currentWeapon, offset + Vec3(0, 0, -.1))
    ...
    def getDrink(self, av, skillId, ammoSkillId, charge, target):
        ...
        return Sequence(Func(self.lockInput, av), Func(hideCurrentWeapon), Func(av.consumable.updateItemId, ammoSkillId), Func(av.consumable.attachTo, av), av.actorInterval('drink_potion', playRate = 1.5, startFrame = 8, endFrame = 45, blendInT = 0.2, blendOutT = 0.2), Func(showCurrentWeapon), Func(av.consumable.detachFrom, av), Func(self.unlockInput, av))
