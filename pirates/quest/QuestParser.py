...
class NPCMoviePlayer(DirectObject.DirectObject):
    ...
    def parseLine(self, command, line, chapterList, timeoutList, lineNum = -1):
        ...
        if command == 'CALL':
            ...
            iList.append(Func(self.acceptOnce, subtitleEvent, self.playNextChapter, [subtitleEvent]))
        ...
        if self.isLocalToon:
            if command == 'LOAD':
                ...
                for currNextEvent in nextEvents:
                    iList.append(Func(self.accept, currNextEvent, handleEvent, [proceed, nextEvents]))
    ...
    def parseDemoCompassIconShow(self, line):
        ...
        if len(line) > 1:
            if line[1] == 'exit0':
                return Sequence(Func(localAvatar.guiMgr.radarGui.flashRadarObject, 'exit-0'), fadeInFunc)
            elif line[1] == 'exit1':
                return Sequence(Func(localAvatar.guiMgr.radarGui.flashRadarObject, 'exit-1'), fadeInFunc)
            elif line[1] == 'localAvatar':
                return Sequence(Func(localAvatar.guiMgr.radarGui.flashRadarObject, localAvatar.getDoId()), fadeInFunc)
            elif line[1] == 'friend':
                targetObjId = 'friend'
                ...
                pirate.generateHuman(style.gender, base.cr.human)
                ...
                localAvatar.compassPirate = pirate
                pos = pirate.getPos(render)
                objectType = RadarGui.RADAR_OBJ_TYPE_TUTORIAL
                teamId = PiratesGlobals.VILLAGER_TEAM
            elif line[1] == 'enemy':
                targetObjId = 'enemy'
                ...
                localAvatar.compassSkeleton = skeleton
                pos = skeleton.getPos(render)
                objectType = RadarGui.RADAR_OBJ_TYPE_TUTORIAL
                teamId = PiratesGlobals.UNDEAD_TEAM
            else:
                targetObj = self.getVar(line[1])
                targetObjId = targetObj.doId
    ...
    def parseEquipWeapon(self, line):
        ...
        def equipCurrWeapon(weapon):
            ...
            localAvatar.guiMgr.combatTray.toggleWeapon(weapon)

        return Func(equipCurrWeapon, weapon)
