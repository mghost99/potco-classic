...
class ActorNodeMgr:
    ...
    def _processStateChange(self):
        ...
        elif self._wantPhysics and 1 in self._loadedZoneLevels:
...
class DistributedShip(DistributedMovingObject, DistributedCharterableObject, ZoneLOD.ZoneLOD, Teamable):
    ...
    def __init__(self, cr):
        ...
        if not self.BountyIcon and base.config.GetBool('want-bountyicons', 0):
            ...
            self.BountyIcon = [
                gui.find('**/ship_pvp_icon_french'),
                gui.find('**/ship_pvp_icon_spanish')]
    ...
    def getDimensions(self, other = None):
        return (VBase3(self.port.getX(other), self.bow.getY(other), self.keel.getZ(other)), VBase3(self.starboard.getX(other), self.stern.getY(other), self.peak.getZ(other)))
    ...
    def startDeployEffect(self, startT = 0):
        ...
        if not self.fogIval:
            duration = 8
            fadeTime = 6
            self.fogIval = Sequence(Wait(5),
                                    Parallel(self.fog.getParticleInterval(duration = duration, fadeTime = fadeTime),
                                    Sequence(Func(self.setDeployHidden),
                                    LerpColorScaleInterval(self, fadeTime, Vec4(1), Vec4(0.001), blendType = 'easeIn'))),
                                    Func(self.clearDeployHidden), Func(messenger.send, 'deployEffectComplete-%d' % self.doId))
    ...
    def registerBuildCompleteFunction(self, func, extraArgs = [], extraKwArgs = {}):
        if self.buildComplete:
            ...
            self.buildCompleteFunctions.append([
                func,
                extraArgs,
                extraKwArgs])
    ...
    def registerMainBuiltFunction(self, func, extraArgs = [], extraKwArgs = {}):
        if self.mainPartsBuilt:
            ...
            self.mainBuiltFunctions.append([
                func,
                extraArgs,
                extraKwArgs])
    ...
    def spawnInAnim(self, speed = 1.0):
        ...
        self.fadeIval = Sequence(Func(self.root.show),
                                 LerpFunctionInterval(self.root.setAlphaScale, fromData = 0.0, toData = 1.0, duration = speed),
                                 Func(self.root.clearTransparency))
        self.fadeIval.start()
    ...
    def spawnOutAnim(self, speed = 1.0):
        ...
        self.fadeIval = Sequence(LerpFunctionInterval(self.root.setAlphaScale, fromData = 1.0, toData = 0.0, duration = speed),
                                 Func(self.root.hide),
                                 Func(self.root.clearTransparency))
        self.fadeIval.start()
    ...
    def loadZoneLevel(self, level):
        if level == 0:
            ...
            currentState = self.gameFSM.getCurrentOrNextState()
            if not (currentState == 'Sunk') and not (currentState == 'Sinking'):
                self.root.unstash()
    ...
    def setShipClass(self, val):
        ...
        if not self.locators:
            ...
            if self.locators:
                ...
                self.notify.warning('failed to load locators for %s(%s,%s)' % (self.doId, self.shipClass, filePrefix))
    ...
    def b_setLocation(self, parentId, zoneId):
        ...
        if self.isGenerated():
            if hasattr(self.cnode, 'setCurrL'):
    ...
    def getDebugNamePosScale(self):
        if self.modelClass == ShipGlobals.INTERCEPTORL1 and self.modelClass == ShipGlobals.MERCHANTL1 or self.modelClass == ShipGlobals.WARSHIPL1:
            return [
                (0, 0, 150),
                50.0]
        elif self.modelClass == ShipGlobals.INTERCEPTORL2 and self.modelClass == ShipGlobals.MERCHANTL2 or self.modelClass == ShipGlobals.WARSHIPL2 or self.modelClass == ShipGlobals.SKEL_INTERCEPTORL3:
            return [
                (0, 0, 250),
                100.0]
            ...
            return [
                (0, 0, 350),
                125.0]
        elif self.modelClass == ShipGlobals.INTERCEPTORL4 and self.modelClass == ShipGlobals.MERCHANTL4 or self.modelClass == ShipGlobals.WARSHIPL4:
            return [
                (0, 0, 450),
                150.0]
    ...
    def getClosestBoardingPosH(self):
        ...
        return (closestBoardingSpot.getPos() + Vec3(1, 0, 0), closestBoardingSpot.getH() + 180)
    ...
    def loadFlat(self):
        ...
        for mastIndex in self.masts:
            ...
            elif self.getBaseTeam() == PiratesGlobals.NAVY_TEAM:
                mastGeom.findAllMatches('**/*_a').detach()
                mastGeom.findAllMatches('**/*_a_*').detach()
                mastGeom.findAllMatches('**/*_c').detach()
                mastGeom.findAllMatches('**/*_c_*').detach()
            elif self.getBaseTeam() == PiratesGlobals.TRADING_CO_TEAM:
                mastGeom.findAllMatches('**/*_a').detach()
                mastGeom.findAllMatches('**/*_a_*').detach()
                mastGeom.findAllMatches('**/*_b').detach()
                mastGeom.findAllMatches('**/*_b_*').detach()
    ...
    def getKrakenSinkCamIval(self):
        ...
        return Parallel(Func(camera.reparentTo, self.attachNewNode('cameraDummy')), Sequence(LerpPosInterval(camera, 12.0, camEndPos, startPos = camStartPos, blendType = 'easeInOut'),
                                                                                             LerpPosInterval(camera, 8.0, camEndPos2, blendType = 'easeInOut')),
                                                                                             LerpPosInterval(self.lookAtDummy, 18.0, Vec3(10, 0, 20)), LerpFunc(camLookAtDummy, 18.0))

    def getSinkShipIval(self):
        return Parallel(
            LerpPosInterval(self.root, 18.0, Vec3(0, 0, -1.5 * self.dimensions[2])),
            LerpHprInterval(self.root, 12.0, VBase3(self.root.getH(), self.root.getP() - 75, self.root.getR())))

    def getKrakenSinkShipIval(self):
        self.splitShip()
        gap = self.dimensions[1] / 2.0
        return Parallel(Sequence(LerpPosInterval(self.root, 0.2, Vec3(0, 0, 1.0)),
                                 LerpPosInterval(self.root, 5.0, Vec3(0, 0, -0.15 * self.dimensions[2])),
                                 LerpPosInterval(self.root, 13.0, Vec3(0, 0, -1 * self.dimensions[2]))),
                        Sequence(LerpPosHprInterval(self.clipParent1, 0.2, Vec3(0, 2, 0), Vec3(0, -3, 0)),
                                 LerpPosHprInterval(self.clipParent1, 8.0, Vec3(0, gap / 2.5, 0), Vec3(0, 40, 0)),
                                 LerpPosHprInterval(self.clipParent1, 6.0, Vec3(0, gap / 1.5, 0), Vec3(0, 50, 0))),
                        Sequence(LerpPosHprInterval(self.clipParent2, 0.2, Vec3(0, -2, 0), Vec3(0, 3, 0)),
                                 LerpPosHprInterval(self.clipParent2, 8.0, Vec3(0, -gap / 2.5, 0), Vec3(0, -40, 0)),
                                 LerpPosHprInterval(self.clipParent2, 6.0, Vec3(0, -gap / 1.5, 0), Vec3(0, -50, 0))))
