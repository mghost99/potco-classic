...
ControlShapes = {
  ...
  'headHeight': [
    [
      ['def_trs_forehead', TZ, 0.566, 0, 0, 0],
      ['def_trs_left_forehead', TZ, 0.47, 0, 0, 0],
  ...
  'headRoundness': [
    [
      ['def_trs_left_jaw1', TX, 0.108, 0, 0, 0],
      ['def_trs_left_jaw1', TZ, -0.095, 0, 0, 0],
      ['def_trs_right_jaw1', TX, -0.108, 0, 0, 0],
  ...
  'jawChinAngle': [
    [
      ['def_trs_left_jaw1', TY, -0.17, 0, 0, 0],
      ['def_trs_right_jaw1', TY, -0.17, 0, 0, 0],
  ...
  'jawChinSize': [
    [
      ['def_trs_left_jaw1', TZ, -0.1, 0, 0, 0],
      ['def_trs_right_jaw1', TZ, -0.1, 0, 0, 0],
  ...
  'mouthLipThickness': [
    [
      ['trs_lips_top', SZ, 1.25, 0, 0, 0],
      ['trs_lips_bot', SZ, 1.25, 0, 0, 0]
    ],
    [
      ['trs_lips_top', SZ, 1.25, 0, 0, 0],
      ['trs_lips_bot', SZ, 1.25, 0, 0, 0]
  ...
  'browProtruding': [
    [
      ['trs_left_eyebrow', TY, -0.02, 0, 0, 0],
  ...
  'eyeOpeningSize': [
    [
      ['trs_left_eyesocket', SX, 1.025, 0, 0, 0],
      ['trs_left_eyesocket', SZ, 1.025, 0, 0, 0],
      ['trs_right_eyesocket', SX, 1.025, 0, 0, 0],
...
class PirateFemale(DirectObject.DirectObject):
    ...
    def setupBody(self, lodName = '2000'):
        ...
        self.currentBody = NodePathCollection()
        self.bodyTextures = loader.loadModelCopy('models/misc/female_body.bam')
        ...
        self.faceTextures = loader.loadModelCopy('models/misc/female_face.bam')
    ...
    def setBeltTexture(self, beltIdx, texIdx, beltPieces):
        ...
        if len(clothes_textures['BELT'][beltIdx]) == 0:
            return
        ...
        tex = self.clothesTextures.findTexture(texName[0])
        self.setClothesTexture(tex, [
            beltPieces[0]], self.clothingsLayer2, lowLODColor)
        if len(beltPieces) > 2:
            self.setClothesTexture(tex, [
                beltPieces[2]], self.clothingsLayer2, lowLODColor)
        
        tex = self.clothesTextures.findTexture(texName[1])
        self.setClothesTexture(tex, [
            beltPieces[1]], self.clothingsLayer2, lowLODColor)
    ...
    def setSockTexture(self, sockIdx, texIdx, sockPieces):
        ...
        texName = clothes_textures['SOCK'][sockIdx][texIdx][0]
        tex = self.clothesTextures.findTexture(texName)
        self.setClothesTexture(tex, sockPieces, self.clothingsLayer1)
    ...
    def handleClothesHiding(self):
        ...
        if vestIdx[0] <= 2 and coatIdx[0] != 3:
    ...
    def handleClothesHidingOld(self):
        ...
        parts = NodePathCollection()
        if self.clothesTextures != None:
            self.setShoeTexture(shoeIdx[0], shoeIdx[1], layerShoe[0])
            ...
            for lod in ['2000', '1000', '500']:
                ...
                for style2 in ['coat', 'noCoat']:
                    for style3 in ['vest1', 'vest2', 'neither']:
                        ...
                        for j in range(data.getNumPaths()):
                            nc = data[j]
        ...
        for beltIdx in range(len(self.clothingsBelt)):
            ...
            for lod in ['2000', '1000', '500']:
                ...
                if len(belt[0]) > 2:
                    ...
                    for i in range(pieceSet3.getNumPaths()):
                        piece = pieceSet3[i]
                        name = piece.getName()
                        beltData['full'][0].addPath(piece)
                        if name.find('_cloth') < 0 and name.find('interior') < 0:
                ...
                for style in ['full', 'coat3']:
                    for i in [0, 1]:
                        data = beltData[style]
                        flattenNode = NodePath('flatten Me')
                        for j in range(data[i].getNumPaths()):
                            nc = data[i][j]
                            nc.copyTo(flattenNode)
                ...
                for lod in ['2000', '1000', '500']:
                    ...
                    for partIdx in hairParts:
                        ...
                        if hatIdx == 0 or partIdx == 14:
                            ...
                            if not cutFound:
                                if hatIdx == 8 and partIdx != 4 or partIdx == 2:
                                    hairIndices.add(partIdx)
                                    hairData.addPathsFrom(hair)
