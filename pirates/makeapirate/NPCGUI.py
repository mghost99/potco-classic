...
class NPCGUI(DirectFrame, StateData.StateData):
    ...
    def setupButtons(self):
        ...
        self.djcrButton = self._makeButton(text = PLocalizer.SkeletonDJ1, command = self.handlecr, pos = (0.05, 0, -1.75), parent = self.shapeFrameTitle)
        self.djjmButton = self._makeButton(text = PLocalizer.SkeletonDJ2, command = self.handlejm, pos = (0.05, 0, -2.25), parent = self.shapeFrameTitle, text_scale = (0.18, 0.2, 0.2))
        self.djkoButton = self._makeButton(text = PLocalizer.SkeletonDJ3, command = self.handleko, pos = (0.05, 0, -2.75), parent = self.shapeFrameTitle)
        self.djpaButton = self._makeButton(text = PLocalizer.SkeletonDJ4, command = self.handlepa, pos = (0.05, 0, -3.25), parent = self.shapeFrameTitle)
        self.djtwButton = self._makeButton(text = PLocalizer.SkeletonDJ5, command = self.handletw, pos = (0.05, 0, -3.75), parent = self.shapeFrameTitle)
