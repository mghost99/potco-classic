...
class MiniScoreItemGui(DirectFrame.DirectFrame):
    ...
    def _createIface(self):
        ...
        if self.world != None:
            ...
            scoreText = ''
        self.descText = DirectLabel(parent = self, relief = None, text = scoreText, text_align = TextNode.ALeft, text_scale = PiratesGuiGlobals.TextScaleLarge, text_fg = textFg, text_shadow = PiratesGuiGlobals.TextShadow, textMayChange = 1, pos = (0.04, 0, 0.015), text_font = PiratesGlobals.getPirateOutlineFont())
        ...
        if self.blink:
            ...
            textPosX = textPos[0]
            textPosY = textPos[1]
            self.scaleSeq = Parallel(Sequence(
                LerpFunctionInterval(self.descText.setScale, duration = 0.2, toData = 1.05, fromData = 1.0, blendType = 'easeInOut'),
                LerpFunctionInterval(self.descText.setScale, duration = 0.7, toData = 1.0, fromData = 1.05, blendType = 'easeInOut')),
                Sequence(Func(self.colorLabel, (1, 1, 1, 1)),
