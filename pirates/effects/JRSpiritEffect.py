...
class JRSpiritEffect(PooledEffect, EffectController):
    ...
    def createTrack(self):
        ...
        uvScrollA = LerpFunctionInterval(self.setNewUVs, duration / 2.5, toData = -2.6, fromData = 0.0, extraArgs = [
            self.effectModel,
            textureStage])
        uvScrollB = LerpFunctionInterval(self.setNewUVs, duration / 4.5, toData = -6.5, fromData = -2.6, extraArgs = [
            self.effectModel,
            textureStage])
