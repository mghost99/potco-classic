...
class Swamp(Water):
    ...
    def camTask(self, task):
        ...
        if self.shader and self.todMgr and self.todMgr.getCurrentOrNextState() != 'Off':
            self.clear_color = self.todMgr.fog.getColor()
            fog_color = self.todMgr.fog.getColor()
            self.fog_r = fog_color.getX()
            self.fog_g = fog_color.getY()
            self.fog_b = fog_color.getZ()
            self.fog_exp_density = self.todMgr.fog.getExpDensity()
            ambient_color = self.todMgr.alight.node().getColor()
            diffuse_color = self.todMgr.dlight.node().getColor()
            ...
            heading = self.todMgr.skyGroup.sunWheel.getH()
